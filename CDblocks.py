...
class Game:
    ...
    def reset_game(self) -> None:
        ...
        # Lock delay / Stall variables
        self.lock_timer = 0
        self.lock_delay_frames = 30  # 0.5 seconds at 60 FPS before locking
        self.lock_resets = 0
        self.max_lock_resets = 15    # Maximum times you can stall a piece before it forces a lock
    ...
    # --- TRUE COLLISION METHODS ---
    def is_colliding(self, test_angle: float, test_r: float, blocks: list) -> bool:
        """
        Checks if the piece blocks placed at 'test_r' and 'test_angle' overlap
        any solid blocks on the wheel.
        """
        target_i = round((90 - test_angle) / SLOT_ANGLE) % N_SLOTS
        eps = 0.001  # Small epsilon to handle floating point perfect alignments
        
        for ds, dl in blocks:
            slot = (target_i + ds) % N_SLOTS
            block_r_in = test_r + dl * LAYER_WIDTH
            block_r_out = block_r_in + LAYER_WIDTH
            
            # Find the grid layers this physical block occupies
            min_L = math.floor((block_r_in + eps - INNER_RADIUS) / LAYER_WIDTH)
            max_L = math.floor((block_r_out - eps - INNER_RADIUS) / LAYER_WIDTH)
            
            for L in range(min_L, max_L + 1):
                if L < 0: return True         # Hit the center core
                if L >= LAYERS: continue      # Above the board is safe air
                if self.wheel[L][slot] is not None:
                    return True               # Hit a locked block
                    
        return False

    def get_drop_r(self, angle: float, blocks: list, start_r: float) -> float:
        """Calculates the lowest grid-aligned resting spot for the piece directly below it."""
        current_layer = int(math.ceil((start_r - INNER_RADIUS) / LAYER_WIDTH))
        
        # Scan down layer by layer to see where it hits
        for L in range(current_layer, -2, -1):
            test_r = INNER_RADIUS + L * LAYER_WIDTH
            if self.is_colliding(angle, test_r, blocks):
                return INNER_RADIUS + (L + 1) * LAYER_WIDTH
    ...
    def attempt_rotate(self) -> None:
        """Rotates the piece, applying wall kicks if the rotation causes a collision."""
        if self.current_piece.shape_id == 'O':
            return
            
        original_blocks = list(self.current_piece.blocks)
        self.current_piece.rotate()

        # Kick tests: (slot offset, layer offset)
        kick_tests = [(0, 0), (1, 0), (-1, 0), (2, 0), (-2, 0), (0, 1), (0, 2)]
        
        success = False
        for d_slot, d_layer in kick_tests:
            test_angle = (self.wheel_angle - d_slot * SLOT_ANGLE) % 360
            test_r = self.current_piece.r + d_layer * LAYER_WIDTH
            
            # Ask the true spatial collision function if this kick spot works
            if not self.is_colliding(test_angle, test_r, self.current_piece.blocks):
                self.wheel_angle = test_angle
                self.current_piece.r = test_r
                success = True
                break
                
        if success:
            self.audio.play('rotate')
            # Reset lock delay if we successfully rotated while on the ground
            if self.lock_timer > 0 and self.lock_resets < self.max_lock_resets:
                self.lock_timer = 0
                self.lock_resets += 1
        else:
            # Revert rotation if all kicks failed
            self.current_piece.blocks = original_blocks

    def lock_piece(self, base_slot: int) -> None:
        # Determine the exact integer grid layer it's resting on
        base_layer = int(round((self.current_piece.r - INNER_RADIUS) / LAYER_WIDTH))
        highest = base_layer + max(dl for _, dl in self.current_piece.blocks)
        ...
        for ds, dl in self.current_piece.blocks:
            ...
            self.wheel[base_layer + dl][slot] = self.current_piece.color
    ...
    def update(self) -> None:
        ...
        if kb_delta != 0:
            ...
            if abs(self.accumulated_keys) >= SLOT_ANGLE:
                ...
                new_angle = (self.wheel_angle + steps * SLOT_ANGLE) % 360
                
                # Verify side-collision before turning the wheel!
                if not self.is_colliding(new_angle, self.current_piece.r, self.current_piece.blocks):
                    self.wheel_angle = new_angle
                    self.accumulated_keys -= steps * SLOT_ANGLE
                    wheel_moved = True
                else:
                    # Bumped a wall. Cap accumulator so it tucks perfectly when a gap opens.
                    self.accumulated_keys = max(-2*SLOT_ANGLE, min(2*SLOT_ANGLE, self.accumulated_keys))
        else:
            self.accumulated_keys = 0.0
            ...
            if self.last_mouse_angle is not None:
                ...
                if abs(self.accumulated_drag) >= SLOT_ANGLE:
                    ...
                    if not self.is_colliding(new_angle, self.current_piece.r, self.current_piece.blocks):
                        ...
                        self.accumulated_drag = max(-2*SLOT_ANGLE, min(2*SLOT_ANGLE, self.accumulated_drag))
        ...
        # --- Stall (Lock Delay Reset) ---
        if wheel_moved and self.lock_timer > 0 and self.lock_resets < self.max_lock_resets:
            self.lock_timer = 0
            self.lock_resets += 1
        ...
        # 1. Update the ghost piece / floor position 
        self.target_r = self.get_drop_r(self.wheel_angle, self.current_piece.blocks, self.current_piece.r)

        # 2. Check if applying fall speed hits the floor
        if self.current_piece.r - self.fall_speed <= self.target_r:
            self.current_piece.r = self.target_r
            self.lock_timer += 1
            if self.lock_timer >= self.lock_delay_frames:
                target_i = round((90 - self.wheel_angle) / SLOT_ANGLE) % N_SLOTS
                self.lock_piece(target_i)
        else:
            self.current_piece.r -= self.fall_speed
            self.lock_timer = 0 # Fell off a cliff into a deeper hole, reset stall!
                    ...
                    if not self.game_over:
                        if event.button == 3:
                            ...
                            if dist < INNER_RADIUS:
                                self.current_piece.r = self.target_r
                                self.lock_timer = self.lock_delay_frames # Force lock immediately
